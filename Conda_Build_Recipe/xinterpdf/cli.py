...
#        self.homepage.bind("<Button-1>", self.callback)
...
##this is the top frame, we will write a few sentences about this program##
        ...
        ttk.Label(self.frame_header, wraplength = 600,
                  text = ("This program is designed to extract structural information from measured X-ray pair distribution function data for organic materials. Currently it supports (1) The study of intermolecular interaction (e.g. hydrogen bonds) by subtracting the scattering signal of a single molecule. (2) The PDF model fit of a crystalline orgainc X-ray PDF using the method proposed by Prill et al. (J. Appl. Cryst., 2015, 48, 171-178.). The homepage for xINTERPDF is at https://github.com/curieshicy/xINTERPDF. The program is developed by Chenyang Shi at AbbVie Inc."
...
class InterPDF():
    ...
    def reset_to_default_cryst(self):
        ...
        self.qmax_2.set(20.0)
        self.delta2_cryst.set(0.0)
        self.atom_den.set(self.atom_den.set(loadStructure(self.input_cryst_struct).occupancy.sum()/loadStructure(self.input_cryst_struct).lattice.volume))
        self.cryst_pc_dpc.set("PDFCalculator")
...
class PDF_model_fit_Uiso():
    def __init__(self, master):
        ...
        self.combobox.grid(row = 8, column = 4, columnspan = 2, padx = 5, sticky = "sw")
        
        ttk.Button(self.top_frame, text = "Run The FIT", command = self.run_the_fit,
                             style = "TButton").grid(row = 8, column = 6, columnspan = 3, padx = 5)
        ttk.Button(self.top_frame, text = "Reset To Default", command = self.reset_to_default,
                             style = "TButton").grid(row = 8, column = 9, columnspan = 3, padx = 5, sticky = "sw")
