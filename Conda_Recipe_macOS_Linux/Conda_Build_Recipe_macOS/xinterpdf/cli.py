...
class Overall_Look:
    def __init__(self, master):
        ...
        self.pdffit_menu.add_command(label = "Breakdown of Fit", command = self.breakdown_fit)
        self.menubar.add_cascade(label = "PDF Fit", menu = self.pdffit_menu)
        
        ##Dropdown menu 4
        self.utility_menu = Menu(self.menubar, tearoff = 0)
        self.utility_menu.add_command(label = "Linear Combination", command = self.linear_comb)
        self.utility_menu.add_command(label = "PCA", command = self.pca)
        self.menubar.add_cascade(label = "Utilities", menu = self.utility_menu)
...
class Linear_Comb():
    def __init__(self, master):
        ...
        self.master.configure(background = "grey91") #the color will be changed later
        self.master.minsize(850, 600) # width + height
        self.master.resizable(False, False)
        ...
        ttk.Label(self.master, text = "Find scale factors that minimize difference/maximize similarity between s0*[s1*PDF-A + s2*PDF-B + (1-s1-s2)*PDF-C] and PDF-D", font = ("Arial", 16, "bold"), justify = CENTER).pack(side = TOP)
        ...
        ttk.Label(self.top_frame, text = "Rmin", justify = CENTER, font = ("Arial", 16, "bold")).grid(row = 3, column = 0, columnspan = 1, padx = 5, sticky = "sw")
        
        ttk.Entry(self.top_frame, width = 8,font = ("Arial", 12),
                  textvariable = self.rmin).grid(row = 3, column = 1, columnspan = 1, padx = 5, sticky = "sw")
                  
        ttk.Label(self.top_frame, text = "Rmax", justify = CENTER, font = ("Arial", 16, "bold")).grid(row = 3, column = 2, columnspan = 1, padx = 5, sticky = "sw")
        
        ttk.Entry(self.top_frame, width = 8,font = ("Arial", 12),
                textvariable = self.rmax).grid(row = 3, column = 3, columnspan = 1, padx = 5, sticky = "sw")
                
        ttk.Label(self.top_frame, text = "Rstep", justify = CENTER, font = ("Arial", 16, "bold")).grid(row = 3, column = 4, columnspan = 1, padx = 5, sticky = "sw")
        
        ttk.Entry(self.top_frame, width = 8,font = ("Arial", 12),
                  textvariable = self.rstep).grid(row = 3, column = 5, columnspan = 1, padx = 5, sticky = "sw")
        
        ttk.Label(self.top_frame, text = "Select Optimizers", justify = CENTER, font = ("Arial", 16, "bold")).grid(row = 3, column = 6, columnspan = 1, padx = 5, sticky = "sw")
...
class Break_down_fit():
    def __init__(self, master):
        ...
        ttk.Label(self.master, text = "Based on the fit result, calculate intra- and inter-part of the fit.", font = ("Arial", 16, "bold"), justify = CENTER).pack(side = TOP)
        self.top_frame = ttk.Frame(self.master, padding = (10, 10))
        self.top_frame.pack()
        ...
        ttk.Label(self.top_frame, text = "Step 2: Make and Save the Plot", justify = CENTER, font = ("Arial", 16, "bold")).grid(row = 2, column = 0, columnspan = 3, padx = 5, sticky = "sw")
...
class PCA():
    def __init__(self, master):
        ...
        ttk.Label(self.master, text = "Generate Score Plot and Scree Plot based on Principle Component Analysis", font = ("Arial", 16, "bold"), justify = CENTER).pack(side = TOP)
        self.top_frame = ttk.Frame(self.master, padding = (10, 10))
        self.top_frame.pack()
        ...
        ttk.Label(self.top_frame, text = "Rmin", justify = CENTER,
                   font = ("Arial", 16, "bold")).grid(row = 1, column = 3, columnspan = 1, padx = 5, sticky = "sw")
        ttk.Entry(self.top_frame, width = 8, font = ("Arial", 12),
                   textvariable = self.rmin).grid(row = 1, column = 4, columnspan = 1, padx = 5, sticky = "sw")

        ttk.Label(self.top_frame, text = "Rmax", justify = CENTER,
                  font = ("Arial", 16, "bold")).grid(row = 1, column = 5, columnspan = 1, padx = 5, sticky = "sw")
        ttk.Entry(self.top_frame, width = 8, font = ("Arial", 12),
        ...
        ttk.Label(self.top_frame, text = "Legend marker size", justify = CENTER,
                  font = ("Arial", 16, "bold")).grid(row = 1, column = 9, columnspan = 1, padx = 5, sticky = "sw")
        ttk.Entry(self.top_frame, width = 8, font = ("Arial", 12),
    ...
    def generate_PCA_plot(self):
        ...
        for x_point, y_point, label in zip(self.data_transform[:,0], self.data_transform[:,1], self.pretty_name_list):
            plt.plot(x_point, y_point, next(markers_database), markersize = int(self.legend_marker_size.get()), label = label)
